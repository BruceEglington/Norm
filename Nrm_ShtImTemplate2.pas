...
begin
  ...
  SheetData.Row := 1;
  SheetData.Col := 1;
  FromRowValueString := UpperCase(meFromRow.Text);
  ToRowValueString := UpperCase(meToRow.Text);
  ePositionCol.Text := UpperCase(ePositionCol.Text);
  eRequiredCol.Text := UpperCase(eRequiredCol.Text);
  eEnteredCol.Text := UpperCase(eEnteredCol.Text);
  eColumnCol.Text := UpperCase(eColumnCol.Text);
  eFactorCol.Text := UpperCase(eFactorCol.Text);
  PositionColStr := ePositionCol.Text;
  RequiredColStr := eRequiredCol.Text;
  EnteredColStr := eEnteredCol.Text;
  ...
  RequiredCol := ConvertCol2Int(eRequiredCol.Text);
  EnteredCol := ConvertCol2Int(eEnteredCol.Text);
  ...
  begin
    try
      ...
      j := EnteredCol;
      tmpVariableID2 := Xls.GetStringFromCell(i,j);
      j := ColumnCol;
      ...
      dmNrm.cdsNormsFacENTERED.AsString := tmpVariableID2;
  ...
  sbSheet.Panels[1].Text := 'Finished importing data definitions';
  sbSheet.Refresh;
end;
...
begin
  ...
  meFromRow.Text := FromRowValueString;
  meToRow.Text := ToRowValueString;
  ePositionCol.Text := PositionColStr;
  eRequiredCol.Text := RequiredColStr;
  eEnteredCol.Text := EnteredColStr;
...
procedure TfmSheetImportTemplate.MatchElementsInFile;
var
  tmpStr : string;
begin
  with dmNrm do
...
begin
  ...
  try
    ...
    j := ConvertCol2Int('A');
    try
      j := ConvertCol2Int(PositionColStr);
    except
      j := ConvertCol2Int('A');
    end;
    ToRow := 0;
